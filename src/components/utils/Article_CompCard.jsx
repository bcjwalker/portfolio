...
export default function CompCard( props ) {
    ...
    const renderThumbButton = ( comp, id ) => {
        return (
            // If our
            <button className={`${styles['thumb-btn']} ${activeComp == id ? `${styles['active']}` : null }`} 
    ...
    const compBtnList = compEntries.map ( (link, index) =>
        renderThumbButton(compEntries[index][1], link[0])
    );
        ...
        <div className={`project-compare-container ${styles['compare-container']}`}>
                ...
                <figure className={`media-wrapper extra-margins ${styles['compare-media-wrapper']}`}>
                    <div className='video-wrapper bordered outline-3'>
                        <ReactCompareSlider transition="0.2s ease-out" onPointerDown={() => setLabelOpacity(0)} onPointerUp={() => setLabelOpacity(1)}
                        // Handle w/ before/after labels
                        handle={
                        <div style={{ display: 'flex', alignItems: 'center', height: '100%'}}>
                            <ReactCompareSliderHandle 
                                buttonStyle={{
                                    boxShadow: '0px 1px 2px 0px hsla(090, 20%, 20%, 0.25), 0px 1px 2px 1px hsla(090, 20%, 20%, 0.125)',
                                    borderWidth: 3,
