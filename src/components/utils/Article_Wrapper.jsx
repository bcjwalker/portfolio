...
function RenderProjectThumbTag(props) {
  return (
    <>
      <span className="metadata-tag">{props}</span>
    </>
  )
}
...
function Header(props) {
  ...
  const tagsList = tagsProps.map((project, index) => RenderProjectThumbTag(tagsProps[index]))
        ...
        <div className="header-main">
          <div className="header-main-left-wrapper">
            <div className={`header-topdtls`}>
              ...
              <h1
                lang="de"
                style={{ viewTransitionName: `post-title-${props.article}` }}
                className={`post-title-${props.article}`}>
...
function RenderLinkList(props) {
  if (props != 'none') {
    ...
    const linkList = linkKeys.map((link, index) => RenderLink(linkKeys[index][1]))
    return linkList
  } else {
    return
  }
...
export function LeadCard(props) {
  ...
  const imgSkeleton = () => {
    return (
      <>
        <div className="lead-icon-placeholder"></div>
      </>
    )
  }

  // 1: Important collaborators
  if (props.cardType == 'Collaborators') {
          ...
          <div className="collab-wrapper">
            <div className="lead-icon">
              <Suspense fallback={imgSkeleton}>
                <img src={projectMedia[`${props.thumb.slice(0, -4)}-thumb.jpg`]} />
              </Suspense>
            </div>
            <label>{props.name}</label>
          </div>
    ...
    return (
      <>
        <div className="lead-card">
          ...
          <div className="collabs-container container">{collabsList}</div>
        </div>
      </>
    )
  }
  ...
  if (props.cardType == 'My roles')
    return (
      <>
        <div className="lead-card">
          <h4>{props.cardType}</h4>
          <div className="roles-container container">{rolesList}</div>
        </div>
      </>
    )
  ...
  if (props.cardType == 'Project stack')
    return (
      <>
        <div className="lead-card">
          <h4>{props.cardType}</h4>
          <div className="software-container container">{softwareList}</div>
        </div>
      </>
    )

  // 4: Timeline
  if (props.cardType == 'Timeline')
    return (
      <>
        <div className="lead-card">
          <h4>{props.cardType}</h4>
...
export function Article_Wrapper({
  ...
  useEffect(() => {
    ...
    let threshold = 5
    let position = articleRef.current.offsetTop + topPos
    ...
    const updateheaderSticky = () => {
      ...
      console.log(scrollY)
      setHeaderSticky(lastScrollY < scrollY && scrollY > position ? false : true)
  ...
  useEffect(() => {
    articleTopRef.current.scrollIntoView({ behavior: 'instant' })
  }, [])
  ...
  return (
    <>
      ...
      <article
        className={`project-big-open ${article}`}
        ref={articleRef}
